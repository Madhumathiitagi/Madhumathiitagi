💫 About Me

🎓 Computer Science Engineering Student from VTU
💻 Passionate about Java Full Stack Development
🚀 Skilled in developing responsive and scalable web applications
🌱 Currently learning Advanced Spring Boot, Microservices & System Design
🧠 Interested in Software Engineering, Backend Development & AI Technologies
📚 Continuously improving Data Structures & Problem Solving Skills
⚡ Love building impactful real-world projects and modern UI experiences

🌐 Connect With Me
💻 Tech Stack
👨‍💻 Programming Languages
🎨 Frontend Development
⚙ Backend Development
Core Java
JDBC & Servlets
Spring Boot
Spring Security
Hibernate / JPA
RESTful APIs
Microservices Architecture
☁ Cloud & DevOps
AWS Basics (EC2, S3)
Docker
Jenkins
Maven
CI/CD Basics
🧠 Core Computer Science
Data Structures & Algorithms
Operating Systems
Computer Networks
DBMS
🤖 AI & LLMs
Basics of Large Language Models (LLMs)
Prompt Engineering
ChatGPT
Google Gemini
Claude Sonnet
Ollama
🚀 Featured Projects
🍦 Vinayaka Icecreams
Modern Dessert Ordering Web Application

✨ A responsive and modern dessert ordering platform built using React + Vite with smooth UI and engaging user experience.

🔹 Key Features
Responsive modern UI
Authentication pages
Cart & checkout functionality
Interactive product sections
Smooth navigation and animations
🛠 Technologies Used

React.js • JavaScript • HTML5 • CSS3 • Vite

🔗 Live Demo: https://vinayaka-icecreams.vercel.app/

🛡 Harmful File Detection System
Machine Learning Based Security Project

Developed a Machine Learning based system capable of identifying whether uploaded files are harmful or safe.

🔹 Key Features
File safety prediction
Machine learning based classification
Data preprocessing and feature extraction
Secure file analysis workflow
🛠 Technologies Used

Python • Machine Learning • Scikit-learn

🛒 SpayRa – Full Stack E-Commerce Web Application

Developed a scalable e-commerce application with secure transaction handling and backend integration.

🔹 Key Features
RESTful API development
Backend services using Spring Boot
Payment gateway integration
Docker-based deployment setup
🛠 Technologies Used

Java • Spring Boot • MySQL • Docker • REST APIs

💼 Experience
Full Stack Java Intern — JSpiders

📅 July 2026 – Present

Developed backend applications using Java and Spring Boot.
Designed RESTful APIs for efficient client-server communication.
Worked with MySQL databases and CRUD operations.
Utilized Git and Maven for version control and project management.
Participated in debugging, testing, and application enhancement.
🎓 Education
Bachelor of Engineering (B.E.) in Computer Science & Engineering

📍 Rao Bahadur Y Mahabaleshwarappa Engineering College
🎓 Affiliated to VTU
📅 2022 – Present
⭐ CGPA: 8.82

📜 Certifications
✅ The Complete Full Stack Web Development Certification – Udemy
✅ Deep Learning with Java – Infosys Springboard
✅ AWS Fundamentals – Infosys Springboard
📊 GitHub Analytics
✨ Developer Mindset

“Building software is not just writing code — it's creating impactful digital experiences.”
